...
## Remove samples with coverage lower than larvae (will be completed after)
```{r}
...
dataCOI$Barcode <- gsub("-", "_", dataCOI$Barcode)

## Compute mean across the three replicates
dataCOI <- dataCOI %>% 
  dplyr::group_by(observation_name, Barcode) %>% 
  dplyr::summarise(CountMean = round(mean(count))) %>% 
  dplyr::ungroup()
dataCOI <- dataCOI %>% 
  tidyr::pivot_wider(names_from = Barcode, values_from = CountMean)
