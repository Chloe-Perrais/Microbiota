...
data16S_family=data16S_family %>% 
  select(c(7,grep("observation_sum",colnames(data16S_family)):ncol(data16S_family)))%>% 
  select(!starts_with("CTL"))%>% 
  select(-Undetermined)

data16S_family <- 
  data16S_family %>%
  dplyr::select(-observation_sum) %>%
  dplyr::mutate(observation_sum=rowSums(select(., !contains("family"))))
