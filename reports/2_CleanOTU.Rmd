...
write.table(abundance_table_clean, file=file.path(here::here(), "data", "derived_data", "2022_COI_JRLAT_cleaned_abundanceandtaxo.txt"), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
```
# Update metadata
```{r }
...
prop_table_clean <- prop_table_clean %>%
    ...
    tidyr::spread_(key = names(prop_table_clean)[1],value = 'value')
## Simplify the name
...
write.table(metadata2, file=file.path(here::here(), "data", "derived_data", "Metadata_withSpecies.txt"), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
```