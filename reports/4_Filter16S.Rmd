...
```{r }
## Import clean data
abundance_table <- data.table::fread(file=file.path(here::here(), "data", "derived_data", "2022_16S_JRLAT_cleaned_abundance.tsv"),sep="\t",header=TRUE)
...
ggplot(covdepthbysample, aes(x=covdepth)) + geom_histogram() + geom_vline(aes(xintercept = 7000), colour="red")
...
## Good correspondence between observed and expected
data.frame(CT1, ExpectFreq=theoreticalproportions$Prop, Species=Species )
```
...
## Good correspondence between observed and expected
data.frame(CT2, ExpectFreq=theoreticalproportions$Prop, Species=Species )
```
